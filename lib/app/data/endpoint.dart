...
import 'data_endpoint/listplanning.dart';
...
class API {
  ...
  static const _getListplanning = '$_baseUrl/get-planning-service';
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == false) {
          ...
          if (obj.token != null) {
            ...
            // Cek data karyawan
            if (obj.data != null && obj.data!.karyawan != null) {
              // Simpan posisi (jika ada)
              if (obj.data!.karyawan!.posisi != null) {
              ...
              // Simpan ID karyawan (misalnya field-nya "id")
              if (obj.data!.karyawan!.id != null) {
                // Pastikan tipenya string, kalau integer, konversi ke string
                await LocalStorages.setKaryawanId(
                    obj.data!.karyawan!.id.toString());
                print(
                    'ID karyawan berhasil disimpan di local: ${obj.data!.karyawan!.id}');
              }
            }

            Get.snackbar('Selamat Datang', 'Mekanik',
...
//Beda
  ...
  //Beda
  static Future<ListPlanning> ListPlanningService() async {
    try {
      final token = Publics.controller.getToken.value ?? '';
      print('Token: $token');
      ...
      final obj = ListPlanning.fromJson(response.data);
